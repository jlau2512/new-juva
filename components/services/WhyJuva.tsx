...
export default function WhyJuva({
  label,
  title,
  items,
}: {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      ctx = gsap.context(() => {
        ScrollTrigger.batch('.js-why-item', {
          start: 'top 88%',
          onEnter: (els) =>
            gsap.fromTo(
              els,
      ...
      <div className="container-x relative">
        ...
        <div className="mt-14 grid gap-px overflow-hidden rounded-3xl border border-white/[0.08] bg-white/[0.06] sm:grid-cols-2">
          {items.map((c, k) => (
            <div
              key={k}
              className="js-why-item group relative flex flex-col bg-bg-card p-8 transition-colors duration-500 hover:bg-bg-card/60 md:p-10"
            >
